...
export default function MotorcycleBrandsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    if (!q) return motorcycleBrands;
    return motorcycleBrands.filter(
      (b) =>
  ...
  const grouped = useMemo(() => {
    const map = new Map<string, typeof motorcycleBrands>();
    ...
    <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6">
      <header className="mb-8">
        ...
        <p className="mt-2 text-muted-foreground">
          {motorcycleBrands.length} motorcycle brands with logos and histories.
        </p>
      </header>
      ...
      <div className="mb-6">
        <div className="relative">
          <input
            ...
            placeholder="Search motorcycle brands…"
      ...
      <p className="mb-6 text-sm text-muted-foreground">
        Showing <span className="font-medium text-foreground">{filtered.length}</span> motorcycle brands
      </p>
