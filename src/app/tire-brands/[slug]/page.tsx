...
export function generateStaticParams() {
  return tireBrands.map((b) => ({ slug: b.slug }));
}

export function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  return params.then((p) => {
    const brand = tireBrands.find((b) => b.slug === p.slug);
    if (!brand) return { title: "Brand not found" };
    return { title: `${brand.name} Logo — History`, description: brand.metaDescription };
  });
}

export default async function TireBrandDetailPage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
  const { slug } = await params;
  const brand = tireBrands.find((b) => b.slug === slug);
  if (!brand) notFound();

  const content = loadSectionContent("tire-brand-content", slug);
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-10 sm:px-6">
      <nav className="mb-6 text-sm text-muted-foreground">
        ...
        <Link href="/tire-brands/" className="hover:text-foreground">Tire Brands</Link>
        <span className="mx-2">/</span>
      ...
      <div className="grid gap-8 md:grid-cols-[280px_1fr]">
        ...
        <div>
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            <span className="rounded-full bg-accent/10 px-3 py-1 text-xs font-medium text-accent">Tire Brands</span>
          </div>
          {brand.description && (
